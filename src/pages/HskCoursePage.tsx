...
function ClassImagesSection() {
  return <section className="bg-brand-cream px-6 py-16 sm:py-20" style={{ scrollMarginTop: '88px' }}><div className="mx-auto max-w-6xl"><SectionHeading label="Không gian học tập">Hình Ảnh Lớp Học</SectionHeading><div className="mt-12 grid items-start gap-6 sm:grid-cols-12 sm:gap-8">
    <figure className="sm:col-span-5"><div className="relative border border-brand-gold/60 bg-white p-2 shadow-xl shadow-brand-red/10 sm:translate-y-6"><CornerOrnaments /><img src="https://res.cloudinary.com/qugyphlv/image/upload/v1789005178/hoat-dong_2.jpg" alt="Lớp học sôi động tại ThanhMaiHSK" className="h-[280px] w-full object-cover sm:h-[370px]" /></div><figcaption className="mt-8 border-l-2 border-brand-gold pl-4 font-sans text-sm font-semibold text-brand-red">Lớp học sôi động, tương tác cao</figcaption></figure>
    <figure className="sm:col-span-4"><div className="relative border border-brand-gold/60 bg-white p-2 shadow-xl shadow-brand-red/10"><img src={classroomImage} alt="Học viên luyện thi HSK" className="h-[300px] w-full object-cover sm:h-[430px]" /></div><figcaption className="mt-4 text-center font-sans text-sm font-semibold text-brand-red">Tập trung vào từng kỹ năng</figcaption></figure>
    <figure className="sm:col-span-3 sm:translate-y-14"><div className="relative border border-brand-gold/60 bg-white p-2 shadow-xl shadow-brand-red/10"><img src="https://res.cloudinary.com/qugyphlv/image/upload/v1789256928/khoa-hoc-hsk.webp" alt="Giáo trình luyện đề HSK" className="h-[260px] w-full object-cover sm:h-[340px]" /></div><figcaption className="mt-4 text-right font-sans text-sm font-semibold text-brand-red">Luyện đề sát cấu trúc thi thật</figcaption></figure>
  </div></div></section>;
}
...
function CommitmentSection() {
  return <section className="bg-brand-cream px-6 py-16 sm:py-20" style={{ scrollMarginTop: '88px' }}><div className="mx-auto max-w-4xl"><SectionHeading label="Điểm tựa trên hành trình">Cam Kết & Giáo Trình</SectionHeading><div className="mt-10 grid gap-6 sm:grid-cols-2 sm:gap-8"><article className="rounded-tl-3xl rounded-br-3xl border border-brand-gold/50 bg-gradient-to-br from-white to-brand-cream p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold-deep hover:shadow-[0_14px_30px_rgba(139,30,30,0.12)]"><DiamondIcon icon={BookMarked} /><h3 className="font-display text-2xl text-brand-red">Giáo Trình Độc Quyền</h3><p className="mt-4 font-sans leading-relaxed text-gray-600">Biên soạn riêng bởi giảng viên nhiều năm kinh nghiệm, bám sát format đề thi mới nhất của Hanban.</p></article><article className="relative rounded-tr-3xl rounded-bl-3xl border-2 border-brand-gold bg-white p-8 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(186,117,23,0.2)]"><div className="absolute -top-3 right-6 bg-brand-red px-4 py-1 font-sans text-xs font-bold uppercase tracking-wide text-brand-gold shadow-md">Cam Kết</div><DiamondIcon icon={ShieldCheck} /><h3 className="font-display text-2xl text-brand-red">Cam Kết Đầu Ra</h3><p className="mt-4 font-sans leading-relaxed text-gray-600">Cam kết đỗ nếu học và ôn thi đầy đủ theo lộ trình. Trượt kỹ năng nào, học lại kỹ năng đó miễn phí.</p></article></div></div></section>;
}
...
function FaqCourseSection() {
  ...
  return <section className="relative overflow-hidden bg-brand-red px-6 py-20 sm:py-28" style={{ scrollMarginTop: '88px' }}><div className="pointer-events-none absolute -right-16 top-8 select-none font-display text-[18rem] leading-none text-white/[0.035]" aria-hidden="true">問</div><div className="relative z-10 mx-auto max-w-3xl"><SectionHeading label="Giải đáp thắc mắc" light>Câu Hỏi Thường Gặp Về HSK</SectionHeading><div className="mt-10 border-t border-brand-gold/25">{faqItems.map((item, index) => { const isOpen = openIndex === index; return <div key={item.q} className="border-b border-brand-gold/25"><button type="button" onClick={() => setOpenIndex(isOpen ? null : index)} className="flex w-full items-center justify-between gap-6 py-6 text-left font-sans text-base font-semibold text-brand-ivory transition-colors hover:text-brand-gold focus:outline-none sm:text-lg" aria-expanded={isOpen}><span>{item.q}</span><ChevronDown className={`h-5 w-5 shrink-0 text-brand-gold transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} /></button><div className="grid transition-[grid-template-rows] duration-300" style={{ gridTemplateRows: isOpen ? '1fr' : '0fr' }}><div className="overflow-hidden"><p className="border-l border-brand-gold/60 pb-6 pl-4 pr-10 font-sans leading-relaxed text-white/70">{item.a}</p></div></div></div>; })}</div></div></section>;
}
